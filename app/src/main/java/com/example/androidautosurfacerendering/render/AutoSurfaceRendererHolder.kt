...
class AutoSurfaceRendererHolder(
    ...
    private val renderer : AutoSurfaceRenderer = AutoSurfaceRenderer()
    ...
    private val me: WeakReference<AutoSurfaceRendererHolder> = WeakReference(this)
    ...
    fun onCreate() {
        if (AutoSurfaceRendererHolder.LOG_ATTACH_DETACH) {
            Log.e(AutoSurfaceRendererHolder.TAG, "onAttachedToWindow reattach =$mDetached")
        }
        if (mDetached) {
    ...
    fun onDestroy() {
        if (AutoSurfaceRendererHolder.LOG_ATTACH_DETACH) {
            Log.e(AutoSurfaceRendererHolder.TAG, "onDetachedFromWindow")
        }
        if (mGLThread != null) {
